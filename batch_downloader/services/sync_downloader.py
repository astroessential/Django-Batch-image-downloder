...
class SyncDownloadService:
    """Synchronous service for downloading images without async complications"""
    ...
    def download_image(self, image_item: ImageItem) -> Tuple[bool, str]:
        """
        Download a single image item synchronously
        ...
        try:
            ...
            existing_file = self._check_existing_file(image_item)
            if existing_file:
                return True, f"File already exists: {existing_file}"
            
            # Download the image
            success, message = self._perform_download(image_item)
    ...
    def _check_existing_file(self, image_item: ImageItem) -> Optional[str]:
        """Check if file already exists by checksum"""
    ...
    def _perform_download(self, image_item: ImageItem) -> Tuple[bool, str]:
...
# Global instance
sync_download_service = SyncDownloadService()
